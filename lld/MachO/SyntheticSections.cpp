...
void InitOffsetsSection::writeTo(uint8_t *buf) const {
  ...
  // FIXME: Add function specified by -init when the argument is implemented.
...
// initializer functions, therefore all relocations have to be of the UNSIGNED
// type. Offsets can only point to local functions, so we may have to generate
// stubs.
